...
W3C 모델에서는 이벤트 객체의 `stopPropagation()` 메소드를 호출하여, 이벤트의 전파를 막을 수 있습니다. `stopPropagation`은 **현재 이벤트 이후로 이벤트의 전파를 막습니다**.

---

# ❌ 이벤트 취소하기 `e.preventDefault()`

`preventDefault()` 메소드를 호출하게 되면, 브라우저가 갖고 있는 **기본적인 이벤트가 취소**되게 됩니다. 단, 이때 **이벤트의 전파는 막지 않습니다**.
