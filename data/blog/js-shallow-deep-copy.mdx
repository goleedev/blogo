...
해당 방법으로 새로운 객체를 `Object.assign()` 메소드를 사용하여 생성하게 되면 **내부의 값을 변경하여도 기존의 객체는 변경되지 않습니다**.
...
`전개 연산자`는 Object.assign() 메소드와 같이 **1차원 객체에서는 깊은 복사가 가능**하지만, **nested 객체에서는 얕은 복사**만 이루어집니다.
...
obj 객체를 `JSON.stringify()` 메소드를 이용하여 문자열로 변환한 뒤, 다시 `JSON.parse()` 메소드로 객체로 변환하였는데, 문자열로 변환한 뒤 다시 객체로 변환하였기에 **2차원 객체에 대한 참조가 사라졌습니다**.

하지만, 이 방법도 2가지 문제가 존재하는데 `1) 다른 방법에 비해 성능이 느리다는 점`과 `2) JSON.stringify() 메소드는 함수를 만났을 때 undefined로 처리된다`는 점입니다.
